...
| `supabase/migrations/237_cleanup_oude_factuur_rpcs.sql` | Cleanup: drop `genereer_factuur_voor_week` + `genereer_factuur(BIGINT[])` + `factuur_queue.type`-kolom |
...
| Pad | Wat verandert |
|---|---|
| `supabase/functions/factuur-verzenden/index.ts` | Switcht van `genereer_factuur` / `genereer_factuur_voor_week` naar `genereer_factuur_voor_bundel(zending_id)`; legacy-fallback voor oude queue-rijen tot na mig 237 |
...
```typescript
      ...
      //      genereer_factuur_voor_week (drained voor mig 237)
      //   3. LEGACY per_zending: zending_id NULL en type='per_zending' →
      //      genereer_factuur (drained voor mig 237)
...
## Task 10: Migratie 237 — Cleanup oude RPCs

**Files:**
- Create: `supabase/migrations/237_cleanup_oude_factuur_rpcs.sql`
...
```sql
-- Migratie 237: cleanup — drop oude factuur-RPCs en factuur_queue.type
--
...
```bash
git add supabase/migrations/237_cleanup_oude_factuur_rpcs.sql
git commit -m "chore(facturatie): drop oude factuur-RPCs + factuur_queue.type — ADR-0010 mig 237"
```
...
```typescript
      // ADR-0010 mig 237: legacy paths gedropt. Alleen genereer_factuur_voor_bundel.
      const { data, error } = await supabase.rpc('genereer_factuur_voor_bundel', {
        p_zending_id: item.zending_id,
      })
      if (error) throw new Error(`RPC genereer_factuur_voor_bundel: ${error.message}`)
...
interface QueueItem {
  ...
  order_ids: number[]
  attempts: number
  zending_id: number  // mig 237: NOT NULL
...
```markdown
## YYYY-MM-DD — ADR-0010 opgeleverd: bundel-driven facturatie live (mig 234-235-237)
...
1. **Mig 235 net toegepast, blijkt fout** → schrijf `mig 237_revert_235.sql`:
...
2. **Mig 237 net toegepast, blijkt fout** → schrijf `mig 238_revert_237.sql` met `genereer_factuur_voor_week` body uit mig 232 git history.
