...
- **`create_inkooporder`-RPC bestaat nog niet** (geverifieerd: geen `FUNCTION create_inkooporder` in het schema-snapshot) — `import/import_inkoopoverzicht.py` doet nog steeds directe table-writes met de service-role-key (RLS-bypass), met een TODO-banner in het script zelf. Zie Openstaand/V2.

## Openstaand / V2

- **Deprecated wrappers opruimen** — `boek_voorraad_ontvangst`/`boek_ontvangst` (mig 271) zijn thin wrappers voor 1 release; streefdatum verwijdering **2026-07-13** (`docs/backlog/inkoop-module-followups.md` punt 1).
- **`create_inkooporder`-RPC** — vervangt `import_inkoopoverzicht.py`'s directe RLS-bypassende writes; ADR-0017 open backlog, nog niet gebouwd.
