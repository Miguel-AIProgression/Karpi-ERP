...
**Cron `verzendbericht-edi-sweep`** (mig 377, */15 min — **NOG NIET TOEGEPAST**): roept edge function `bouw-verzendbericht-edi` aan als sweep over orders met `status='Verzonden' AND bron_systeem='edi'` bij partners met `verzend_uit && transus_actief`, minus al-bestaande `berichttype='verzendbericht'`-rijen (idempotent). De format-builder `_shared/transus-formats/karpi-verzendbericht.ts` gooit bewust een fout totdat het Transus-DESADV-format gevalideerd is (Taak 12-STOP). Migratie pas toepassen na: Taak 12 (voorbeeld downloaden + format bouwen + Transus Testen-tab validatie) → deploy → gerichte test. Verstuurd door bestaande cron `transus-send` (mig 305).
