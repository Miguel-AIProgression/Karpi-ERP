...
| `match_klant_po(p_extractie jsonb) → jsonb` | **Klant-PO parsing — deterministische koppellaag** (mig 289). Matcht AI-extractie van een klant-inkooporder-PDF tegen de database. Debiteur: btw → e-maildomein → exacte naam (telkens precies 1 hit = `zeker`, anders geen debiteur). Per regel: kwaliteit via reverse-lookup op `klanteigen_namen.benaming` (debiteur-/inkoopgroep-scoped) + exacte `kwaliteiten.omschrijving`; kleur via numeriek suffix; artikel via `klant_artikelnummers` / `producten`. Elk veld draagt een `zeker`-label — alleen `zeker`-velden worden door de frontend voorgevuld. STABLE, geen side-effects. GRANT anon/authenticated/service_role. |
