...
| is_handmatig | BOOLEAN DEFAULT false | Sinds migratie 154: true = gebruiker-gekozen allocatie-claim (uitwisselbaar-voorraad óf inkooporder, mig 491-492). Allocator releaset deze claims NIET en telt ze mee als reeds-gedekt. |
| created_at, updated_at | TIMESTAMPTZ | Auto |
...
**Allocator gesplitst, geen automatische substitutie meer (mig 489-494, 2026-06-24):** vóór mig 489 claimde `herallocateer_orderregel` bij een tekort automatisch — zonder gebruikersactie — eerst een uitwisselbaar/equivalent artikel ("omstickeren") en daarna de oudste open inkooporder. Sinds mig 489 doet `herallocateer_orderregel(id)` (de naam die `trg_orderregel_herallocateer` aanroept, dus voor alle intake-kanalen) alleen nog **Stap 1 (eigen voorraad)** — een resterend tekort blijft gewoon tekort totdat een gebruiker expliciet kiest. De volledige oude cascade (Stap 1 → uitwisselbaar-voorraad → oudste IO) leeft voort als **`herallocateer_orderregel_auto(id)`**, aangeroepen ná een bevestigde keuze voor het niet-gekozen restant. Eenmalige backfill (mig 492) gaf alle bestaande automatische alias/IO-claims vrij, behalve op orders die al 'In pickronde' of verder stonden. Zie ook [order-lifecycle.md](order-lifecycle.md) en de CLAUDE.md-bullet "Uitwisselbaar = handmatige claims".
...
| `set_uitwisselbaar_claims(p_order_regel_id BIGINT, p_keuzes JSONB)` | Migratie 154. **@deprecated** sinds mig 492 — geen frontend-call-sites meer (vervangen door `set_allocatie_keuze`); blijft staan tot een opruim-migratie 'm dropt. |
