...
| leverancier_id | BIGINT FK → leveranciers | **Mig 514. Single source of truth voor leverancier op kwaliteit-niveau.** Een leverancier is een eigenschap van de kwaliteit (DREAM = altijd HENAN), niet van individuele kleuren/maten. Wijzigen hier propageert automatisch naar alle producten van die kwaliteit — de UI schrijft via `UPDATE kwaliteiten SET leverancier_id = X`. Beheer: inline dropdown op `/instellingen/kwaliteiten`, bij product aanmaken/bewerken, en product-detail. Backfill (mig 512-514): 63 kwaliteiten gekoppeld via inkooporders + uitwisselgroepen. `backorder_per_artikel` gebruikt `COALESCE(kwaliteiten.leverancier_id, producten.leverancier_id)` als fallback voor producten zonder kwaliteit-koppeling. `producten.leverancier_id` blijft staan als legacy-kolom (niet meer beschreven door UI). |
...
| combi_levering | BOOLEAN NOT NULL DEFAULT false | Mig 556 (ADR-0039). Klant wil wachten met verzenden tot de gecombineerde openstaande orders naar hetzelfde adres de `verzend_drempel` bereiken, i.p.v. direct verzendkosten op een individuele order onder de drempel. Zie view `combi_levering_status` en trigger `trg_debiteuren_combi_levering` (mig 558). |
...
| leverancier_id | BIGINT FK → leveranciers(id) ON DELETE SET NULL | **Mig 487 (2026-06-24).** Default/gebruikelijke leverancier voor dit artikel — puur informatief, geen koppeling met de inkoop-flow (`inkooporders.leverancier_id` is daar de bron-van-waarheid). Vóór deze migratie had de kolom niet bestaan terwijl "+ Nieuw product"/"Bewerken" 'm al wel onvoorwaardelijk in elke create/update-payload stuurden — elke opslag via die twee formulieren faalde daardoor altijd met `42703: column does not exist`, los van enige andere wijziging. |
...
| combi_levering_override | BOOLEAN NOT NULL DEFAULT false | Mig 556 (ADR-0039). Klant wil dít exemplaar toch los verzonden, met verzendkosten, ongeacht `debiteuren.combi_levering`. Instelbaar in het order-form, analoog aan `afhalen`. |
...
| is_handmatig | BOOLEAN DEFAULT false | Sinds migratie 154: true = gebruiker-gekozen allocatie-claim (uitwisselbaar-voorraad óf inkooporder, mig 499-500). Allocator releaset deze claims NIET en telt ze mee als reeds-gedekt. |
| created_at, updated_at | TIMESTAMPTZ | Auto |
...
**Allocator gesplitst, geen automatische substitutie meer (mig 497-502, 2026-06-24):** vóór mig 497 claimde `herallocateer_orderregel` bij een tekort automatisch — zonder gebruikersactie — eerst een uitwisselbaar/equivalent artikel ("omstickeren") en daarna de oudste open inkooporder. Sinds mig 497 doet `herallocateer_orderregel(id)` (de naam die `trg_orderregel_herallocateer` aanroept, dus voor alle intake-kanalen) alleen nog **Stap 1 (eigen voorraad)** — een resterend tekort blijft gewoon tekort totdat een gebruiker expliciet kiest. De volledige oude cascade (Stap 1 → uitwisselbaar-voorraad → oudste IO) leeft voort als **`herallocateer_orderregel_auto(id)`**, aangeroepen ná een bevestigde keuze voor het niet-gekozen restant. Eenmalige backfill (mig 500) gaf alle bestaande automatische alias/IO-claims vrij, behalve op orders die al 'In pickronde' of verder stonden. Zie ook [order-lifecycle.md](order-lifecycle.md) en de CLAUDE.md-bullet "Uitwisselbaar = handmatige claims". **Migratienummers 489-494 waren oorspronkelijk gepland maar botsten met een concurrent sessie (Rhenus/HST-pallet + vertegenwoordiger-rol, ook 489-495) — hernummerd naar 497-502 vóór de merge naar main.**
...
5. Voor elke bundel-zending van die week (`zendingen.verzendweek = p_jaar_week`, status `Klaar voor verzending`+, gekoppeld via `zending_orders` aan een order in de factuur):
   ...
   - INSERT 1 VERZEND-regel met omschrijving zoals `"Verzendkosten 2026-W22 (HST, 2 orders)"` of `"... — gratis vanaf €501,00"`.
...
**Koppeling aan rollen:** `rollen.inkooporder_regel_id` (BIGINT FK → inkooporder_regels) legt vast uit welke regel een fysieke rol ontvangen is. Gevuld door RPC `boek_inkooporder_ontvangst_rollen` (hernoemd van `boek_ontvangst` in mig 271).
...
| Enum | Waarden |
|------|---------|
| order_status | **Klaar voor picken** (mig 257, ADR-0016, default sinds mig 275), **Wacht op maatwerk** (mig 257), Wacht op voorraad, **Wacht op inkoop** (mig 144), **Wacht op combi-levering** (mig 563, ADR-0040 — klant wacht op de vrachtvrije-drempel over meerdere orders; `derive_wacht_status`-ladder, laagste prioriteit, buiten Pick & Ship tot promotie naar Klaar voor picken), **In pickronde** (mig 257), **Deels verzonden** (mig 257), Verzonden, Geannuleerd, **Maatwerk afgerond** (mig 327, ADR-0029 — terminale status uitsluitend voor productie-only orders uit Basta; bereikt zodra alle snijplannen confectie-afgerond zijn; valt buiten Pick & Ship/facturatie/transport). Legacy (niet meer geschreven post-mig 275, behalve `In productie` als import-status voor productie-only orders): Nieuw, Actie vereist, Wacht op picken, In snijplan, In productie, Deels gereed, Klaar voor verzending. |
...
| View | Doel |
|------|------|
| producten_overzicht | Producten + rollen-aggregatie (aantal_rollen, oppervlak, waarde) + locatie + `lengte_cm`/`breedte_cm` (mig 488, 2026-06-24 — voedt vorm-groep/oppervlak-sortering in de frontend, zie `kwaliteit-kleuren-uitvouw.tsx`) |
...
| orderregel_pickbaarheid | Mig 170; mig 288: `'Snijden'`-rang-fix; **mig 386 (v4):** (a) generieke admin-pseudo-skip `AND NOT is_admin_pseudo(oreg.artikelnr)` (ADR-0018) — vervangt de VERZEND-specifieke TS-skip én fixt de latente dropship-blokkade (DROPSHIP-KLEIN/-GROOT-regels kregen geen claim → stonden als `wacht_op='inkoop'` → dropship-orders werden nooit "alles pickbaar"); (b) nieuwe kolom `gewicht_kg` (uit `order_regels`) zodat de aparte gewicht-query in TS vervalt. Per orderregel (open orders, niet-pseudo): `order_regel_id`, `order_id`, `regelnummer`, `artikelnr`, `is_maatwerk`, `orderaantal`, maatwerk-afmetingen, `is_pickbaar`, `bron` (`snijplan`\|`rol`\|`producten_default`\|NULL), `fysieke_locatie`, `wacht_op` (`snijden`\|`confectie`\|`inpak`\|`inkoop`\|NULL), `gewicht_kg`. Single source voor Pick & Ship; de TS-laag leidt niets meer af. **Mig 498 (2026-06-24):** CTE `voorraad_claim` ging van `COUNT(*)` naar `SUM(aantal)` — vóór mig 497 cascadeerde de allocator altijd tot het volledige tekort gedekt was, dus "≥1 claim-rij" en "som ≥ benodigd" waren toen equivalent; sinds mig 497 (geen automatische alias/IO-substitutie meer) kan een regel een ECHTE partiële voorraad-claim hebben, en zou de oude `COUNT(*)>0`-check zo'n regel ten onrechte als volledig pickbaar tonen. `is_pickbaar`/`wacht_op` voor niet-maatwerk vergelijken nu `SUM(aantal) >= te_leveren`. |
| combi_levering_status | Mig 557 (ADR-0039). Puur lezend, geen state — herevalueert per query, spiegelt `voorgestelde_zending_bundels` (mig 229). Per order (alleen klanten met `debiteuren.combi_levering=TRUE`, niet-`combi_levering_override`, niet-dropshipment): `order_id`, `groep_subtotaal` (SUM excl. admin-pseudo-regels over alle openstaande orders van de (debiteur × adres-norm)-groep, `_normaliseer_afleveradres`, géén vervoerder/verzendweek in de sleutel), `verzend_drempel`, `gratis_verzending`, `alle_leden_pickbaar` (BOOL), `wacht_op_combi_levering` (BOOL) = `NOT gratis_verzending AND (groep_subtotaal < COALESCE(verzend_drempel, 500) OR NOT alle_leden_pickbaar)`. Orders die niet aan de WHERE voldoen komen niet in de view voor — consumenten gebruiken `LEFT JOIN + COALESCE(..., FALSE)`. Voedt sinds mig 565 (ADR-0040) de `orders.status`-ladder (`herbereken_wacht_status` leest `wacht_op_combi_levering` en geeft 'm door aan `derive_wacht_status` als 5e parameter — vervangt de eerdere Startbaarheid-laag-consumptie), `PickShipOrder.combi_levering_deelnemer` (aanwezigheid van een rij, mig 562-fix — onderscheidt "nooit deelnemer" van "deelnemer, drempel gehaald") en de orderbevestiging-paragraaf. Mig 561: sluit `'In pickronde'`/`'Deels verzonden'`-orders uit van het groep-subtotaal (al fysiek onderweg, niets meer om mee te combineren). Mig 562: NULL `verzend_drempel` valt terug op €500 (`SHIPPING_THRESHOLD`, was "geen drempel = altijd gehaald"). Mig 569: twee kolommen erbij (`aantal_orders`, `order_ids` — groepsgrootte + leden), voedt via `orders_list` (nieuwe kolommen `combi_levering_aantal_orders`/`wacht_op_combi_levering`/`combi_levering_andere_orders`, JOIN op `aantal_orders >= 2`) de Combi-levering-badge op orders-overview (`orders-table.tsx`) en order-detail (`order-header.tsx`) — visuele koppeling tussen orders die samen op de vrachtvrije-drempel wachten of 'm net gehaald hebben, los van de fysieke zending-bundel (`bundel_zending_nr`). Gedeelde component `combi-levering-badge.tsx`. **Mig 570 (pre-prod-audit 02-07, blocker 1):** mig 569 herbouwde de view per ongeluk vanaf de verouderde pre-561/562-body — de `'In pickronde'/'Deels verzonden'`-uitsluiting en de `COALESCE(verzend_drempel, 500)`-fallback waren stilletjes verdwenen (kort live gestaan). Mig 570 herstelt beide fixes bovenop de mig 569-kolommen, en sluit nieuw ook `status='Concept'` en `alleen_productie=TRUE`-orders uit van het groep-lidmaatschap (onbevestigde e-mail-intake resp. Basta-orders horen de groep niet te sturen of te blokkeren). |
| order_pickbaarheid | **Mig 386** (hernummerd van 383 via 385; in de live DB op 12-06 onder werknummer 383 toegepast). Aggregaat per order over `orderregel_pickbaarheid`. Kolommen: `order_id`, `totaal_regels` (INT), `pickbare_regels` (INT), `alle_regels_pickbaar` (BOOL), `heeft_pickbare_regel` (BOOL), `deelleveringen_toegestaan` (BOOL, uit `debiteuren`), `pick_ship_zichtbaar` (BOOL — zie mig 476 voor de volledige formule). **Geen rij** = order heeft geen (niet-pseudo) regels = niets te picken. Single source voor het Pick & Ship-orderfilter ([`fetchPickShipOrders`](../frontend/src/modules/magazijn/queries/pickbaarheid.ts)) en de pick-start-knop (`StartPickrondesButton.alle_regels_pickbaar`); alleen de dag-order-horizon (ADR 0014) blijft client-side. **Deploy-voorwaarde:** mig 386 moet op de live DB staan vóór de frontend van deze branch deployt — er is geen fallback meer. **Mig 476 (2026-06-23):** `pick_ship_zichtbaar` = `alle_regels_pickbaar OR (deelleveringen_toegestaan AND heeft_pickbare_regel) OR EXISTS(actieve zending — status IN ('Gepland','Picken') — voor deze order)`. De derde tak is nieuw: een order met een al-lopende pickronde moet zichtbaar blijven in Pick & Ship om afgerond te kunnen worden, ook als de statische pickbaarheid-snapshot (bv. door een override-deelzending bij `deelleveringen_toegestaan=false`, of een regel die ná het starten van de pickronde niet meer pickbaar bleek) anders zou zeggen "onzichtbaar". Geverifieerd: voor orders zonder actieve zending een no-op. **Mig 479 (2026-06-23):** nieuwe kolom `heeft_gepland_zending` (BOOL, losse EXISTS alleen op `status='Gepland'`) — voedt de frontend-startbaarheid (`bepaalStartbaarheid`/`startbaarheid.ts`): die blokkeerde de "Picken starten"-knop op `!alle_regels_pickbaar` volledig los van een al-klaarstaande Gepland-zending, waardoor mig 477's promotielogica in `start_pickronden` onbereikbaar bleef voor exact het scenario waarvoor ze gebouwd was. **Mig 566 (ADR-0040, 2026-07-01):** `pick_ship_zichtbaar` krijgt een extra `AND o.status <> 'Wacht op combi-levering'`-guard (zelfde stijl als mig 521's manco-guard) op de reguliere tak — de eerste `orders.status`-filter in deze view; nooit van toepassing op de actieve-zending-OR-tak (een combi-levering-wachtende order is per definitie nooit gestart). |
...
| `herallocateer_orderregel(p_order_regel_id BIGINT)` | **Centrale seam**, aangeroepen door `trg_orderregel_herallocateer` (alle intake-kanalen). Idempotent: release alle actieve niet-handmatige claims + alloceer opnieuw — **sinds mig 497 alleen Stap 1 (eigen voorraad)**, geen automatische uitwisselbaar/IO-substitutie meer. Resterend tekort blijft tekort tot een gebruiker kiest (`set_allocatie_keuze`) of het via `ontgrendel_allocatie_keuze` weer hierheen terugvalt. Sluit maatwerk en regels zonder artikelnr uit. Migratie 145, gesplitst mig 497. |
| `herallocateer_orderregel_auto(p_order_regel_id BIGINT)` | Mig 497: de **volledige oude cascade** van `herallocateer_orderregel` (Stap 1 eigen voorraad → Stap 1.5 uitwisselbaar-voorraad → Stap 2 oudste open IO via `verwacht_datum ASC`). Wordt alleen nog aangeroepen door `set_allocatie_keuze`, voor het niet-gekozen restant ná een bevestigde keuze — niet door de intake-trigger. |
...
| `set_uitwisselbaar_claims(p_order_regel_id BIGINT, p_keuzes JSONB)` | Migratie 154. **@deprecated** sinds mig 500 — geen frontend-call-sites meer (vervangen door `set_allocatie_keuze`); blijft staan tot een opruim-migratie 'm dropt. |
...
| `allocatie_opties_voor_artikel(p_artikelnr TEXT)` | Mig 499, kolom `eigen_artikelnr` toegevoegd mig 501. Pure, herevaluerende RPC — live databron voor de 3-opties-keuze bij een tekort: (1) eigen artikel z'n open IO-regels met ETA, (2) equivalent (zelfde `collectie_id`+`kleur_code`+afmetingen+`maatwerk_vorm_code` als de allocator z'n eigen Stap 1.5-matching) nu op voorraad, (3) equivalent z'n eigen open IO-regels met ETA. `eigen_artikelnr` staat constant op elke rij zodat de frontend zonder de doos→stuks-vertaling (mig 408) te herhalen kan groeperen. Returns TABLE(`bron`, `artikelnr`, `omschrijving`, `inkooporder_regel_id`, `vrij_aantal`, `verwacht_datum`, `eigen_artikelnr`). |
| `set_allocatie_keuze(p_order_regel_id BIGINT, p_keuzes JSONB)` | Mig 500. Vervangt **alle** actieve claims (handmatig én niet-handmatig) van de orderregel door de in `p_keuzes` opgegeven `[{bron, artikelnr, aantal, inkooporder_regel_id?}]`-lijst (`is_handmatig=true`, dus vergrendeld) en roept daarna `herallocateer_orderregel_auto` aan voor het niet-gekozen restant. Een IO-keuze die `io_regel_ruimte()` overschrijdt → `RAISE EXCEPTION`, geen partiële insert. |
| `ontgrendel_allocatie_keuze(p_order_regel_id BIGINT)` | Mig 500. Release alle `is_handmatig=true`-claims van de orderregel en valt terug op de **korte** `herallocateer_orderregel` (bewust niet `_auto` — anders triggert ontgrendelen meteen weer een nieuwe automatische claim). Mirrort `ontgrendel_handmatige_toewijzing` (mig 453, snijplanning). |
| `handmatige_keuzes_voor_order(p_order_id BIGINT)` | Migratie 239, uitgebreid mig 502 met `bron`/`inkooporder_regel_id`/`verwacht_datum`. Alle actieve, handmatige allocatie-keuzes van een order (uitwisselbaar-voorraad én IO-claims), met fysiek-product-omschrijving erbij. Voedt de edit-mode-hydratatie (`hydrateerOrderRegels`) — zonder `bron` zou een eerder gekozen IO-claim bij een ongewijzigde re-save stil herschreven worden naar een (foutieve) voorraad-claim. |
...
| `update_order_with_lines(p_order_id BIGINT, p_header JSONB, p_regels JSONB)` | Merge-update van order header + regels: UPDATE bestaande regels op `id`, INSERT nieuwe, DELETE regels die uit payload verdwenen zijn. Preserveert `snijplannen.order_regel_id` FK-koppelingen (migratie 074). **Mig 572 (pre-prod-audit 02-07, blocker 3):** roept aan het eind altijd `herbereken_wacht_status(p_order_id)` aan — dekt ook een prijs-/korting-only-edit of regel-verwijdering, die `trg_orderregel_herallocateer` (mig 146, vuurt alleen op `artikelnr`/`te_leveren`/`is_maatwerk`) niet triggert. Bij een adres- of debiteurwijziging roept het daarna óók `herbereken_combi_groep` aan voor de VERLATEN (oude) groep. |
...
| `boek_inkooporder_ontvangst_rollen(p_regel_id BIGINT, p_rollen JSONB, p_medewerker TEXT DEFAULT NULL, p_sta_overlevering_toe BOOLEAN DEFAULT FALSE)` | **Hernoemd van `boek_ontvangst` in migratie 271** (pure rename, ADR-0017; oude naam bestond tot mig 604 als deprecated thin wrapper). Atomair: maakt N rollen aan op basis van `[{lengte_cm, breedte_cm, rolnummer?, locatie?}, ...]`, logt `voorraad_mutaties` (type=`'inkoop'`, referentie_type=`'inkooporder_regel'`), werkt `geleverd_m`/`te_leveren_m` bij (boekt **m²**, niet strekkende meters — fix migratie 133) en zet order-status op 'Deels ontvangen'/'Ontvangen'. Alleen voor eenheid='m'. Rolnummer optioneel — leeg = auto-genereer `R-YYYY-NNNN` via `volgend_nummer('R')` (migratie 135). **Mig 603 (2026-07-02):** (1) optioneel `locatie` per rol → `create_or_get_magazijn_locatie` (mig 169) → `rollen.locatie_id`; (2) pre-pass telt de totale payload-m² en weigert >110% van besteld tenzij `p_sta_overlevering_toe=TRUE` (tikfout-vangnet — over-levering zelf is normaal in tapijt); (3) `v_product` nu onvoorwaardelijk toegewezen (latente crash-fix) en karpi_code-only regels (`artikelnr IS NULL`, mogelijk sinds mig 601) weigeren met heldere melding "Koppel eerst een artikel..." (`rollen.artikelnr` is NOT NULL+FK; `producten.karpi_code` niet uniek — geen auto-resolutie). Returns TABLE(rol_id, rolnummer). Superset-keten 127→133→135→136→271→281→603. |
| `boek_inkooporder_ontvangst_stuks(p_regel_id BIGINT, p_aantal INTEGER, p_medewerker TEXT)` | **Hernoemd van `boek_voorraad_ontvangst` in migratie 271** (pure rename, ADR-0017; oude naam bestond tot mig 604 als deprecated thin wrapper). Voor vaste producten (eenheid='stuks'): verhoogt `producten.voorraad` met p_aantal en werkt regel + order-status bij. Sinds migratie 148: consumeert IO-claims in `claim_volgorde`-volgorde en verschuift ze naar voorraad-claims op dezelfde orderregel; roept `herwaardeer_order_status` aan per geraakte order. |
| ~~`boek_ontvangst`~~ / ~~`boek_voorraad_ontvangst`~~ | **GEDROPT in mig 604** (⚠ geschreven, nog niet gedraaid op de live DB op 2026-07-02 — pas draaien ná merge+Vercel-deploy, vóór 2026-07-13). Tot dan bestaan ze als deprecated thin wrappers (mig 271) die naar de hernoemde functies hierboven delegeren; alle frontend-/import-callers zijn al omgezet naar de nieuwe namen. |
| `create_inkooporder(p_header JSONB, p_regels JSONB) → TABLE(inkooporder_id, inkooporder_nr)` | **Inkoop-Module (mig 601, 2026-07-02).** Eén schrijfpad voor UI én import: transactioneel aanmaken van inkooporder + regels (vervangt de oude 3-losse-inserts-flow die een lege order kon achterlaten). Status altijd `'Besteld'`; `besteldatum` default `CURRENT_DATE` (COALESCE — kolom is NOT NULL). Regels ondersteunen zowel `eenheid='m'` (rollen) als `eenheid='stuks'` (o.a. antislip, mig 408 — de UI kon dat pad voorheen niet bereiken). |
| `herbereken_inkooporder_status(p_inkooporder_id BIGINT)` | **Mig 602.** Herleidt `inkooporders.status` uit de regels (analoog `herwaardeer_order_status` voor klantorders). No-op bij status `'Geannuleerd'`/`'Concept'` of 0 regels. Aangeroepen door de regel-mutatie-RPC's hieronder. |
| `voeg_inkooporder_regel_toe(p_inkooporder_id BIGINT, p_regel JSONB) → BIGINT` | **Mig 602.** Regel toevoegen aan een bestaande inkooporder ("extra artikel bijbestellen"); `regelnummer` = MAX+1 binnen de order. Swap-evaluatie loopt via de bestaande INSERT-trigger op `inkooporder_regels` (ongewijzigd, ADR-0027). |
| `wijzig_inkooporder_regel(p_regel_id BIGINT, p_besteld NUMERIC DEFAULT NULL, p_inkoopprijs_eur NUMERIC DEFAULT NULL, p_vrijgeven BOOLEAN DEFAULT FALSE)` | **Mig 602.** Aantal/prijs wijzigen met **Claim-vloer**-guard (zie CONTEXT.md/data-woordenboek.md): verlagen onder `geleverd + actieve verkooporder-claims + snijplan-'Wacht op inkoop'-claims` weigert tenzij `p_vrijgeven=TRUE`. Bij vrijgeven: released snijplan-claims + verkooporder-claims via `release_claims_voor_io_regel`, en **force-releaset daarna ook handmatige (`is_handmatig=true`) claims** op die regel (incl. `herwaardeer_order_status` per getroffen order) — nodig omdat de allocator handmatige claims by design nooit loslaat terwijl de IO-dekking hier verdwijnt; defensieve post-check blijft als vangnet. Getroffen klantorders vallen zichtbaar terug naar 'Wacht op inkoop'. |
| `annuleer_inkooporder_regel(p_regel_id BIGINT, p_vrijgeven BOOLEAN DEFAULT FALSE)` | **Mig 602.** "Rest komt niet meer": zet `besteld := geleverd`; delegeert naar `wijzig_inkooporder_regel` (zelfde Claim-vloer-guard). |
| `verwijder_inkooporder_regel(p_regel_id BIGINT, p_vrijgeven BOOLEAN DEFAULT FALSE)` | **Mig 602.** Verwijdert een regel-vergissing (kale DELETE op `inkooporder_regels` is verboden terrein — ADR-0017). Weigert bij ontvangsten (`geleverd_m > 0`) én bij claim-**historie** (`order_reserveringen` is append-only audit met `ON DELETE RESTRICT`) — melding verwijst dan naar "Regel annuleren" i.p.v. verwijderen. Weigert ook de laatste regel van een order. `herbereken_inkooporder_status` no-op't ook op status `'Concept'`. |
...
| `combi_levering_orderregel_subtotaal(p_order_id BIGINT) → NUMERIC` | Mig 557 (ADR-0039). Order-subtotaal excl. admin-pseudo-regels, voor de Combi-levering-drempeltoets. STABLE. Mig 562: uitsluiting omgezet van hardcoded `<> 'VERZEND'` naar het generieke `is_admin_pseudo()` (ADR-0018) — VORMTOESLAG/DROPSHIP-*/BUNDELKORTING/DREMPELKORTING telden voorheen ten onrechte mee. |
| `herwaardeer_combi_levering_verzendregel(p_order_id BIGINT) → VOID` | Mig 558/561/562/574 (ADR-0039/pre-prod-audit 02-07). Voegt/verwijdert de VERZEND-orderregel op een order op basis van `debiteuren.combi_levering`/`orders.combi_levering_override`/`afhalen` en het eigen ordersubtotaal t.o.v. de klant-drempel (NULL-drempel valt terug op €500, mig 562 — mirrort de view en `applyShippingLogic`). Idempotent; aangeroepen door `trg_orders_combi_levering_override` (AFTER UPDATE OF `combi_levering_override` op `orders`) en `trg_debiteuren_combi_levering` (AFTER UPDATE OF `combi_levering` op `debiteuren`, loopt over alle open orders van de klant). Mig 561: no-op voor orders die al `'In pickronde'`/`'Deels verzonden'` zijn. **Mig 574 (hoog):** een dropship-order (`is_dropship_order`) krijgt nooit meer een VERZEND-regel via dit pad, ook niet in het normale (niet-wachtende) pad — de dropship-kostenregel ís al de verzendcomponent (mig 353/370); een al-bestaande VERZEND-regel op een dropship-order wordt bewust niet stilzwijgend verwijderd (handmatige beoordeling). |
| `zet_order_in_combi_levering_wacht(p_order_id BIGINT) → VOID` | Mig 560 (ADR-0039). Order-detail-knop-RPC: zet `debiteuren.combi_levering=TRUE` (klant-breed, alle openstaande orders naar dat adres) en `orders.combi_levering_override=FALSE` voor déze order. Trigger `trg_debiteuren_combi_levering` herwaardeert vanzelf de VERZEND-regels. |
| `herbereken_combi_groep(p_debiteur_nr INTEGER, p_adres_norm TEXT) → VOID` | Mig 572 (pre-prod-audit 02-07, blocker 3). Herevalueert alle Combi-levering-leden van één (debiteur × adres-norm)-groep via `herbereken_wacht_status(order_id, FALSE)` per lid. Consument: `update_order_with_lines`, voor de VERLATEN groep na een adres-/debiteurwijziging op een order (de order zelf zit dan al in de nieuwe groep en kan de oude niet meer via de normale sibling-cascade bereiken). |
