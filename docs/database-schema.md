...
| `combi_levering_orderregel_subtotaal(p_order_id BIGINT) → NUMERIC` | Mig 551 (ADR-0039). Order-subtotaal excl. VERZEND, voor de Combi-levering-drempeltoets — zelfde exclusie als `voorgestelde_zending_bundels.bundel_subtotaal_excl` (mig 229). STABLE. |
| `herwaardeer_combi_levering_verzendregel(p_order_id BIGINT) → VOID` | Mig 552/555 (ADR-0039). Voegt/verwijdert de VERZEND-orderregel op een order op basis van `debiteuren.combi_levering`/`orders.combi_levering_override`/`afhalen` en het eigen ordersubtotaal t.o.v. de klant-drempel. Idempotent; aangeroepen door `trg_orders_combi_levering_override` (AFTER UPDATE OF `combi_levering_override` op `orders`) en `trg_debiteuren_combi_levering` (AFTER UPDATE OF `combi_levering` op `debiteuren`, loopt over alle open orders van de klant). Mig 555: no-op voor orders die al `'In pickronde'`/`'Deels verzonden'` zijn. |
