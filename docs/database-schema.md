...
| combi_levering | BOOLEAN NOT NULL DEFAULT false | Mig 550 (ADR-0039). Klant wil wachten met verzenden tot de gecombineerde openstaande orders naar hetzelfde adres de `verzend_drempel` bereiken, i.p.v. direct verzendkosten op een individuele order onder de drempel. Zie view `combi_levering_status` en trigger `trg_debiteuren_combi_levering` (mig 552). |
...
| combi_levering_override | BOOLEAN NOT NULL DEFAULT false | Mig 550 (ADR-0039). Klant wil dít exemplaar toch los verzonden, met verzendkosten, ongeacht `debiteuren.combi_levering`. Instelbaar in het order-form, analoog aan `afhalen`. |
...
| `combi_levering_orderregel_subtotaal(p_order_id BIGINT) → NUMERIC` | Mig 551 (ADR-0039). Order-subtotaal excl. admin-pseudo-regels, voor de Combi-levering-drempeltoets. STABLE. Mig 556: uitsluiting omgezet van hardcoded `<> 'VERZEND'` naar het generieke `is_admin_pseudo()` (ADR-0018) — VORMTOESLAG/DROPSHIP-*/BUNDELKORTING/DREMPELKORTING telden voorheen ten onrechte mee. |
| `herwaardeer_combi_levering_verzendregel(p_order_id BIGINT) → VOID` | Mig 552/555/556/568 (ADR-0039/pre-prod-audit 02-07). Voegt/verwijdert de VERZEND-orderregel op een order op basis van `debiteuren.combi_levering`/`orders.combi_levering_override`/`afhalen` en het eigen ordersubtotaal t.o.v. de klant-drempel (NULL-drempel valt terug op €500, mig 556 — mirrort de view en `applyShippingLogic`). Idempotent; aangeroepen door `trg_orders_combi_levering_override` (AFTER UPDATE OF `combi_levering_override` op `orders`) en `trg_debiteuren_combi_levering` (AFTER UPDATE OF `combi_levering` op `debiteuren`, loopt over alle open orders van de klant). Mig 555: no-op voor orders die al `'In pickronde'`/`'Deels verzonden'` zijn. **Mig 568 (hoog):** een dropship-order (`is_dropship_order`) krijgt nooit meer een VERZEND-regel via dit pad, ook niet in het normale (niet-wachtende) pad — de dropship-kostenregel ís al de verzendcomponent (mig 353/370); een al-bestaande VERZEND-regel op een dropship-order wordt bewust niet stilzwijgend verwijderd (handmatige beoordeling). |
| `zet_order_in_combi_levering_wacht(p_order_id BIGINT) → VOID` | Mig 554 (ADR-0039). Order-detail-knop-RPC: zet `debiteuren.combi_levering=TRUE` (klant-breed, alle openstaande orders naar dat adres) en `orders.combi_levering_override=FALSE` voor déze order. Trigger `trg_debiteuren_combi_levering` herwaardeert vanzelf de VERZEND-regels. |
| `herbereken_combi_groep(p_debiteur_nr INTEGER, p_adres_norm TEXT) → VOID` | Mig 566 (pre-prod-audit 02-07, blocker 3). Herevalueert alle Combi-levering-leden van één (debiteur × adres-norm)-groep via `herbereken_wacht_status(order_id, FALSE)` per lid. Consument: `update_order_with_lines`, voor de VERLATEN groep na een adres-/debiteurwijziging op een order (de order zelf zit dan al in de nieuwe groep en kan de oude niet meer via de normale sibling-cascade bereiken). |
