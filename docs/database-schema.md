...
| verzendweek_bron | TEXT CHECK IN ('handmatig','automatisch_voorraad') | Mig 469. Herkomst van `verzendweek`: `'handmatig'` = bewust door een operator gezet/aangepast via `set_regel_verzendweek`, `'automatisch_voorraad'` = systeemvoorstel ("vandaag + N weken", N uit `app_config.productie_planning.maatwerk_voorraad_levertijd_weken`, default 7) zodra trigger `trg_snijplan_rol_toegewezen_auto_verzendweek` op `snijplannen` detecteert dat een maatwerk-regel volledig op een echte rol staat. Zet nooit een bestaande waarde terug — snapshot, geen live herberekening. NULL = geen verzendweek gezet. |
...
| `trg_snijplan_rol_toegewezen_auto_verzendweek` (op `snijplannen`, niet `order_regels`) | INSERT OF rol_id, UPDATE OF rol_id | AFTER | Mig 469: `trg_snijplan_rol_toegewezen_auto_verzendweek()` — schrijft naar `order_regels.verzendweek`/`verzendweek_bron` van de bijbehorende maatwerk-regel zodra ALLE snijplan-stukken van die regel een echte `rol_id` hebben, mits `verzendweek` nog NULL is (snapshot, geen overschrijving). |
