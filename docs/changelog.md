...
## 2026-06-13 — Config-constanten naar single source (tech-debt categorie C, mig 388)

> **Nummering:** deze migratie is hernummerd van 387 → **388** vlak vóór de merge (origin/main claimde 387 via de colli-gewicht-fix). In de live DB is de assert op 13-06 al uitgevoerd (nul mismatches); de inhoud is idempotent.
...
**Deploy-voorwaarde:** mig 388 toepassen (read-only assert; valideert dat de live `normaliseer_land` de golden volgt) + edge functions `factuur-verzenden`, `rhenus-send` (en `bouw-factuur-edi` via de gedeelde mapper) herdeployen ná merge.
