...
- `<LevertijdSuggestie />` — gebruikt door order-form in `modules/orders/` zonder dat orders weet wat de planning-database-shape is. Sinds [ADR-0013](adr/0013-snijplanning-module-en-cache-invalidation-seam.md) leeft de logica niet meer in `modules/planning/` (die folder bestond nooit fysiek); de Snijplanning-Module en de Confectie-Module zijn aparte zustermodules.
