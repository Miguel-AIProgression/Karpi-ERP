...
### Rhenus-koppeling: GS1 TransportInstruction-XML via SFTP ([ADR-0032](adr/0032-rhenus-gs1-xml-sftp-adapter.md), mig 379-382 — in de live DB gedraaid als 378-381 vóór de hernummering wegens collisie met main's 378)
...
**Hergebruik bovenop de Verhoek-seams:** `_shared/sftp-client.ts` (verplaatst uit `verhoek-send` — pure file-move, beide adapters importeren hem), `_shared/vervoerder-eisen.ts` (`rhenus_sftp` deelt de SFTP-tak: adresvelden verplicht, telefoon/land niet), dispatch-case `WHEN 'rhenus_sftp'` in de bestaande `'sftp'`-tak (mig 380), `externe_payloads` kanaal `'rhenus'`, storage `order-documenten/rhenus-xml/` (XML-mime in de bucket-allowlist via mig 382), cron-vault-token (mig 381, `rhenus-send-elke-minuut`). De orchestrator-loop is opnieuw **gespiegeld, niet geabstraheerd** — ADR-0031 markeerde de derde vervoerder als generalisatie-moment, maar dat is bewust uitgesteld (cutover-week); alleen de sftp-client is gegeneraliseerd. Expliciet backlog-item.

**Formaat-verschillen t.o.v. Verhoek:** gewichten in **kg met decimalen** (geen decagram), adres als één regel (`streetAddressOne` — geen `splitAdres`), `<sscc>` = de gedeelde **Labelbarcode** (`labelBarcode()`, AI(00)+SSCC, 20 cijfers — sinds 2026-06-14 niet meer via config-vlag), geen T&T-slot in het bericht. Config in `app_config` sleutel `'rhenus'` (`package_type_code` default `'RLEN'`, `bestandsnaam_prefix` `'RHE'`) — per run gelezen.
...
**Cutover-betekenis van `actief=TRUE`:** mig 379 hangt de live selectie-regels van placeholder `edi_partner_a` om naar `rhenus_sftp` (DE ≤30 kg kleinste zijde ≥131 + debiteur-pins 99001/640505). Zodra de vervoerder actief gaat (week 24-afspraak), routeren die regels DE-zendingen automatisch via Rhenus — geen extra regel- of code-werk. Go-live-checklist: zie Fase 2 in het [plan](superpowers/plans/2026-06-12-rhenus-transporteur-gs1-xml-sftp.md).

**Status (2026-06-12, eind van de dag):** migraties toegepast, functions gedeployed, secrets gezet (Piet-Hein — vereist owner/admin), dry-run- én echte rondreis geslaagd: testbestand via de edge-runtime geüpload naar Rhenus' `/test`-map en onafhankelijk geverifieerd. Open: format-akkoord van Rhenus op het testbestand → daarna `RHENUS_SFTP_REMOTE_DIR=/in` + `actief=TRUE` (= cutover). Gewicht-datagap (`zending_colli.gewicht_kg` vrijwel overal 0) moet vóór echte verzending opgelost; de preflight blokkeert dit terecht.
...
### Werkagenda-kernel: geen spiegel meer, maar gedeelde implementatie (2026-06-12, mig 383)

`_shared/werkagenda.ts` is **geen TS-spiegel** van een andere implementatie — het is de enige bron. Zowel de frontend als de edge functions importeren de kernel direct: `bereken-agenda.ts` importeert de kernel via een cross-root-import (vite `server.fs.allow` staat dit toe; patroon: derive-status via gedeelde module). De kernel is **dependency-vrij** — dat is de voorwaarde voor de cross-root-import. De dode SQL-implementatie (mig 279, nul callers) is gedropt via mig 383. Golden fixture `_shared/__tests__/werkagenda.golden.json` wordt door zowel Deno als Vitest getoetst — semantische gelijkheid in beide runtimes is afdwingbaar, niet alleen een belofte. Sortering is bewust niet geünificeerd: `berekenAgenda` (UI) sorteert op leverdatum→kwaliteit→kleur→rolnummer (NULL als vandaag), `berekenSnijAgenda` (kernel) op leverdatum→rolId (NULL last) — zie besluit B6 in de kernel-header.
