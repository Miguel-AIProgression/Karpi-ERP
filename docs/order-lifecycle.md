...
**Eén bevestigd-predicaat:** `isOrderBevestigd(order)` in [`bevestiging-kanaal.ts`](../frontend/src/lib/orders/bevestiging-kanaal.ts) — leest `edi_bevestigd_op` voor EDI-orders en `bevestigd_at` voor de rest. De order-header gebruikt dit predicaat voor badge en knop.

**EDI-orders krijgen nooit e-mail.** Het kanaal hangt aan de order (`bron_systeem`), niet aan de klant — een EDI-klant die via een ander kanaal bestelt (bv. telefonisch) krijgt gewoon de e-mail-orderbevestiging.
