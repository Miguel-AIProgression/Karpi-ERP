...
- **Dynamische bundel-preview + week-dimensie (mig 228-230):** bundel-sleutel is **4-dimensionaal** — `(debiteur × adres-norm × effectieve vervoerder × verzendweek)`. Helper-functies `bundel_sleutel(...)` + `verzendweek_voor_datum(date)` (ISO-week `YYYY-Www`) zijn de single source of truth, gespiegeld in [`bundel-sleutel.ts`](frontend/src/lib/orders/bundel-sleutel.ts). Pure SQL-view `voorgestelde_zending_bundels` (mig 229) levert de live preview vóór pickronde-start: geen state, geen materialized view — herevalueert per query. Frontend cachet via React Query (`['voorgestelde-bundels']`, staleTime 60s) en invalidate't bij vervoerder-override, afleverdatum-mutatie en pickronde-start. Pick & Ship `KlantClusterBlok` toont per bundel een drempel-progressbar (slate/amber/teal) + besparing-badge. `start_pickronden_bundel` valideert nu ook identieke verzendweek; `zendingen.verzendweek` is een snapshot. Trigger `trg_lock_zending_bundel_sleutel` (mig 230) blokkeert mutatie van afleverdatum/afl_*/debiteur op orders die in een actieve bundel-zending zitten (`Klaar voor verzending`+).
