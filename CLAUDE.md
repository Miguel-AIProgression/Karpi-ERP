...
- **Verzendkosten bij gesplitste orders (mig 0182, T007):** verzendkosten-regel (`omschrijving ILIKE '%verzend%' OR artikelnr ILIKE '%VERZEND%'`) belandt op het sub-order met de hoogste totale regelwaarde. Helper: `kies_duurste_suborder(p_suborders jsonb) → INTEGER` (0-gebaseerde index). Routing is **passief klaar** in `commit_order_voorstel` zolang `was_split = false` (T006 limiet); activeert automatisch zodra order-splitsing actief wordt. Result-JSON bevat altijd `verzendkosten_routing: 'duurste_suborder'` als bevestiging. TypeScript-type: `CommitOrderVoorstelResult.verzendkosten_routing?: string`.
