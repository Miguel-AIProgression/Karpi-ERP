...
export function PrijslijstAddKlantDialog({ prijslijstNr, prijslijstNaam, onClose }: Props) {
  const [search, setSearch] = useState('')
  const [selected, setSelected] = useState<Set<number>>(new Set())
  const [error, setError] = useState<string | null>(null)
  const { data: debiteuren, isLoading } = useKoppelbareDebiteurenMetPrijslijst()
  const mutation = useSetKlantenPrijslijst()

  const filtered = useMemo(() => {
    if (!debiteuren) return []
    const list = debiteuren.filter((d) => d.prijslijst_nr !== prijslijstNr)
    ...
    return list
      ...
      .slice(0, MAX_VISIBLE)
  }, [debiteuren, search, prijslijstNr])
  ...
  const verplaatsCount = useMemo(() => {
    ...
    return debiteuren.filter(
      (d) =>
        ...
        d.prijslijst_nr !== null &&
        d.prijslijst_nr !== prijslijstNr,
    ).length
  }, [debiteuren, selected, prijslijstNr])
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError(null)
    if (selected.size === 0) {
      setError('Kies eerst minimaal één klant')
      return
    }
    try {
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          ...
          {verplaatsCount > 0 && (
            <div className="px-6 py-3 bg-amber-50 border-t border-amber-100 text-sm text-amber-800">
              ...
              {verplaatsCount === 1 ? 'klant zit' : 'klanten zitten'} nu nog op een andere
              prijslijst — die koppeling wordt vervangen.
            </div>
          )}
