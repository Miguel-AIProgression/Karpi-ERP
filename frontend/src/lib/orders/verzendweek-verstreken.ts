...
// geannuleerde orders en oud_systeem. Spiegelt hetzelfde filter in fetchOrders /
// fetchStatusCounts. Tegenhanger van de 'Geen verzendweek'-seam (afleverdatum NULL).
// NB: alleen kolommen die zowel op `orders` als op de view `orders_list` bestaan —
// `alleen_productie` zit alléén op `orders`, dus die wordt hier bewust niet gebruikt
// (zou de orders_list-query breken; precies wat de geen-verzendweek-seam ook doet).
