"""Import prijslijst0248 (Begros per 1-6-2026) naar Supabase + koppel 13 debiteuren."""
import sys
import re
import io
...
FILE = BASE_DIR / "prijslijst0248_a.xlsx"
PRIJSLIJST_NR = "0248"
...
def main():
    print("=== Import prijslijst 0248 (Begros) ===\n")
    ...
    meta = rows[1]
    naam = str(meta[2]).strip() if meta[2] else "BEGROS"
    ...
    # 4. Koppel debiteuren
    print(f"\n  {len(DEBITEUREN)} debiteuren koppelen aan prijslijst {PRIJSLIJST_NR}...")
    result = sb.table("debiteuren").select("debiteur_nr, naam, prijslijst_nr").in_("debiteur_nr", DEBITEUREN).execute()
    gevonden = {r["debiteur_nr"]: r for r in result.data}

    niet_gevonden = [d for d in DEBITEUREN if d not in gevonden]
    if niet_gevonden:
        print(f"  WAARSCHUWING: niet gevonden in DB: {niet_gevonden}")

    update_result = sb.table("debiteuren").update({"prijslijst_nr": PRIJSLIJST_NR}).in_("debiteur_nr", DEBITEUREN).execute()
    for r in update_result.data:
        was = gevonden.get(r["debiteur_nr"], {}).get("prijslijst_nr", "?")
        print(f"  {r['debiteur_nr']} ({r['naam']})  {was} → {PRIJSLIJST_NR}")
