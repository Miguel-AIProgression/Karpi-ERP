"""
Sync inkoopoverzicht 25-06-2026 naar DB.

Stappen:
  1. 88 nieuwe inkooporders importeren (excl. Team snijtafel, excl. historisch <2025, excl. 2023)
  2. Vervallen IO's verwijderen (open in RugFlow, niet meer in CSV)
     - reserveringen op die IO-regels eerst loskoppelen
     - daarna regels → orders verwijderen
  3. Leverweek-updates doorvoeren
  4. Te-leveren hoeveelheden bijwerken
  5. Geraakt verkooporder_regels herberekenen via herallocateer_orderregel()
"""
import sys, re, io
...
CSV_PATH   = BASE_DIR / 'Inkoopoverzicht 25-06-2026.csv'
UITGESLOTEN_LEV = {20010}
MIN_JAAR   = 2025  # excl. 2023-orders op verzoek
...
HENAN_LEVERANCIER_NR = 23200  # HENAN BEST INT. TRADINGCO.,LTD — levertijden via portal

def fetch_db_orders():
    rows, offset = [], 0
    while True:
        res = sb.table('inkooporders').select(
            'id,oud_inkooporder_nr,inkooporder_nr,status,leverweek,verwacht_datum,leverancier_id'
...
def fetch_portal_leverancier_ids():
    """Geef de set van leverancier_ids waarvoor het portal leidend is (nu: Henan)."""
    res = sb.table('leveranciers').select('id').eq('leverancier_nr', HENAN_LEVERANCIER_NR).execute()
    return {r['id'] for r in (res.data or [])}

def stap3_leverweek(lw_changes, db_open, portal_lev_ids):
    print(f"\n=== STAP 3: {len(lw_changes)} leverweek-updates ===")
    overgeslagen = 0
    for onr, (io_id, oud_lw, nieuw_lw) in lw_changes.items():
        # Henan-orders niet overschrijven — levertijden worden per container
        # bijgehouden door Zhang Qi via het supplier portal (mig 523).
        # update_regel_eta propageert die regel-datums al naar order-niveau.
        lev_id = db_open.get(onr, {}).get('leverancier_id')
        if lev_id in portal_lev_ids:
            print(f"  {onr}  OVERGESLAGEN (portal-leverancier — leverweek niet overschrijven)")
            overgeslagen += 1
            continue
    ...
    if overgeslagen:
        print(f"  ({overgeslagen} portal-leverancier orders overgeslagen)")
...
def main():
    ...
    print("Laad DB...")
    portal_lev_ids = fetch_portal_leverancier_ids()
    print(f"  Portal-leveranciers (leverweek niet overschrijven): {portal_lev_ids}")
    ...
    stap3_leverweek(lw_changes, db_open, portal_lev_ids)
