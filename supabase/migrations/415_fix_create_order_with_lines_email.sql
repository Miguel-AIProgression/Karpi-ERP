-- Migratie 415: herstel fact_email + afl_email in create_order_with_lines
--
-- Oorzaak: mig 406 (klant_referentie per orderregel) herschreef de RPC
-- create_order_with_lines zonder de fact_email/afl_email-kolommen over te
-- nemen uit mig 364. Alle handmatig aangemaakte orders na mig 406 kregen
-- daardoor fact_email = NULL en afl_email = NULL, ook als de klant wél een
-- e-mailadres had.
--
-- Fix: herschrijf de RPC met alle kolommen compleet (mig 406 + mig 364).
-- Tevens een backfill van bestaande orders zonder fact_email die klanten
-- met een e-mailadres hebben.
...
COMMENT ON FUNCTION create_order_with_lines(JSONB, JSONB) IS
  'Mig 415: herstelt fact_email + afl_email die mig 406 per ongeluk wegliet. '
  'Alle e-mailadressen worden nu correct als snapshot opgeslagen bij aanmaken.';

-- ============================================================================
-- Backfill: vul fact_email voor bestaande orders zonder snapshot,
-- waarvan de klant wél een e-mailadres heeft.
-- ============================================================================
UPDATE orders o
   SET fact_email = COALESCE(d.email_factuur, d.email_overig)
  FROM debiteuren d
 WHERE d.debiteur_nr = o.debiteur_nr
   AND NULLIF(TRIM(COALESCE(o.fact_email, '')), '') IS NULL
   AND COALESCE(d.email_factuur, d.email_overig) IS NOT NULL
