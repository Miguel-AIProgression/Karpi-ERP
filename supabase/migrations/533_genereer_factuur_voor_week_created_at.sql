-- Migratie 533: toeslag-activatie op orders.created_at + toeslag_procent snapshot
--              (wekelijkse factuurpad — spiegelt mig 532 voor het per_zending-pad)
--
-- Superset van mig 530 (genereer_factuur_voor_week_toeslag). Twee wijzigingen:
--
-- 1. ACTIVATIE-CHECK: was CURRENT_DATE BETWEEN begindatum AND einddatum.
--    Nieuw: BOOL_AND(o.created_at::date >= begindatum AND ... <= einddatum)
--    over alle orders in de weekbatch (v_order_ids).
--
-- 2. toeslag_procent SNAPSHOT: facturen.toeslag_procent (mig 531) wordt gevuld
--    met debiteuren.toeslag_procent zodat de PDF "Zuschlag 4,5%" kan tonen.
...
COMMENT ON FUNCTION public.genereer_factuur_voor_week(integer, text) IS
  ...
  'regeling-bewust mig 456, toeslag mig 530, created_at-check + procent-snapshot mig 533. '
