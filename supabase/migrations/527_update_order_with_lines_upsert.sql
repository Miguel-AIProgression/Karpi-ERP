-- Migratie 527: update_order_with_lines — UPSERT i.p.v. DELETE+INSERT
--
-- Aanleiding: orders met een Gepland/Picken-deelzending konden niet bewerkt
-- worden. De RPC deed DELETE FROM order_regels WHERE order_id = ..., maar
-- zending_regels heeft een FK naar order_regels.id — dat gooide:
...
--
-- Fix: UPSERT-aanpak die bestaande order_regel IDs bewaart:
--   1. DELETE regels die niet meer in p_regels zitten
--      → FK-constraint blokkeert bewust als zending_regels nog verwijst
--        (pickronde moet dan eerst afgerond/geannuleerd worden)
--   2. UPDATE bestaande regels in-place (id matcht + bestaat voor deze order)
--      → zending_regels FK blijft intact
--   3. INSERT nieuwe regels (geen id in p_regels, of id bestaat niet)
--
-- Bijvangst: mig 407's v_verzendweken-workaround (snap+herstel per
-- regelnummer) vervalt — UPDATE in-place raakt verzendweek/verzendweek_bron
-- NIET (niet in de SET-clause), dus die blijven automatisch bewaard.
...
BEGIN
    ...
    -- Alleen bestaande regels waarvan het id NIET voorkomt in p_regels worden
    -- verwijderd. Nieuwe regels (null-id in p_regels) zijn nog niet ingevoegd,
    -- dus de subquery raakt ze niet.
    -- Als zending_regels naar zo'n regel verwijst gooit Postgres de FK-fout —
    -- de gebruiker moet de pickronde dan eerst afhandelen.
