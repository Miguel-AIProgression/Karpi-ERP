-- Migratie 356: maatwerk-artikel-koppeling — backfill karpi_code + herstel 3 orderregels
--
...
BEGIN
  ...
  RAISE NOTICE 'Mig 356a: unique index op producten.karpi_code aanwezig: %', v_unique_idx;
  ...
  DROP TABLE IF EXISTS _mig356_kandidaten;
  CREATE TEMP TABLE _mig356_kandidaten ON COMMIT DROP AS
  ...
  SELECT COUNT(*) INTO v_kandidaten FROM _mig356_kandidaten;
  ...
  FROM (
    ...
    FROM _mig356_kandidaten k
    WHERE EXISTS (SELECT 1 FROM producten p2 WHERE p2.karpi_code = k.nieuwe_code)
       OR (SELECT COUNT(*) FROM _mig356_kandidaten k2 WHERE k2.nieuwe_code = k.nieuwe_code) > 1
    LIMIT 20
  ) s;
  ...
  WHERE p.artikelnr = k.artikelnr
    ...
    AND (SELECT COUNT(*) FROM _mig356_kandidaten k2 WHERE k2.nieuwe_code = k.nieuwe_code) = 1;
  GET DIAGNOSTICS v_updated = ROW_COUNT;

  v_geskipt := v_kandidaten - v_updated;
  RAISE NOTICE 'Mig 356a: % kandidaten, % karpi_codes backfilled, % geskipt (duplicaat-guard)',
    v_kandidaten, v_updated, v_geskipt;

  IF v_geskipt > 0 THEN
    RAISE NOTICE 'Mig 356a: geskipte codes (max 20): %', v_skip_voorbeeld;
  END IF;
END $$;
...
BEGIN
  ...
  IF v_order_id IS NULL THEN
    RAISE NOTICE 'Mig 356b: ORD-2026-0118 niet gevonden — skip (niets te herstellen op deze omgeving)';
    RETURN;
  END IF;
  ...
  IF v_artikelnr IS NULL THEN
    RAISE NOTICE 'Mig 356b: product LAGO13MAATWERK niet gevonden — skip artikelnr-herstel ORD-2026-0118';
    RETURN;
  END IF;
  ...
  GET DIAGNOSTICS v_updated = ROW_COUNT;
  RAISE NOTICE 'Mig 356b: ORD-2026-0118 — % regel(s) gekoppeld aan % (LAGO13MAATWERK)', v_updated, v_artikelnr;
END $$;
...
BEGIN
  ...
  IF v_order_id IS NULL THEN
    RAISE NOTICE 'Mig 356c: ORD-2026-0098 niet gevonden — skip (niets te herstellen op deze omgeving)';
    RETURN;
  END IF;
  ...
  GET DIAGNOSTICS v_updated = ROW_COUNT;
  RAISE NOTICE 'Mig 356c: ORD-2026-0098 regel 1 — kwaliteit-split toegepast op % rij(en) (0 = al gefixt)', v_updated;
  ...
  IF v_fout_kwal > 0 THEN
    RAISE EXCEPTION 'FAAL mig 356c: kwaliteit-split niet geland — ORD-2026-0098 regel 1 heeft nog LUXR17';
  END IF;
  ...
  ORDER BY p.artikelnr LIMIT 1;
  IF v_artikelnr IS NULL THEN
    RAISE NOTICE 'Mig 356c: product LUXR17MAATWERK niet gevonden — artikelnr blijft NULL op ORD-2026-0098 regel 1';
    RETURN;
  END IF;
  ...
  WHERE r.order_id = v_order_id
    ...
    AND r.artikelnr IS NULL;
  GET DIAGNOSTICS v_updated = ROW_COUNT;
  RAISE NOTICE 'Mig 356c: ORD-2026-0098 regel 1 — % regel(s) gekoppeld aan % (LUXR17MAATWERK)', v_updated, v_artikelnr;
END $$;
...
BEGIN
  ...
  IF v_rest = 0 THEN
    RAISE NOTICE 'Mig 356: zelf-test OK — geen maatwerk-regels zonder artikelnr meer op ORD-2026-0098/0118';
  ELSE
    RAISE NOTICE 'Mig 356: LET OP — nog % maatwerk-regel(s) zonder artikelnr: % (zie skip-NOTICEs hierboven)', v_rest, v_detail;
  END IF;
END $$;
