-- Migratie 232: genereer_factuur_voor_week — wekelijkse verzamelfactuur
--
-- Sluit de keten van mig 228-231: voor een (debiteur_nr, jaar_week) wordt
-- één factuur gegenereerd met:
--   1. Alle product-regels van Verzonden orders van die week
--   2. Per bundel-zending van die week één VERZEND-regel met drempel-toets
--
-- **Verzendkosten worden per bundel-zending geheven**, niet per week.
-- Reden: een bundel = 1 fysieke transportbeweging. Twee verschillende
-- vervoerders in dezelfde week = 2 transportbewegingen = 2× verzendkosten,
-- mits het bundel-totaal (per bundel) onder de drempel blijft. Op de
-- factuur ziet de klant dus per bundel: "Verzendkosten week 22 (HST,
-- 2 orders)" of "Verzendkosten week 22 (Verhoek, 1 order)".
--
-- Dit volgt de bestaande `genereer_factuur` (mig 227) zoveel mogelijk:
--   · Header-INSERT met dezelfde defaults
--   · Product-regels via dezelfde SELECT-shape
--   · Mig 227 no-op-guard (faal vroeg bij 0 te factureren regels)
-- Toegevoegd:
--   · Verzendweek-filter via verzendweek_voor_datum(orders.afleverdatum)
--   · Per zending van die week een VERZEND-regel met drempel-toets
--   · Eindtotalen rekenen mee met verzending-regels
--
-- Idempotent: CREATE OR REPLACE.

CREATE OR REPLACE FUNCTION genereer_factuur_voor_week(
...
COMMENT ON FUNCTION genereer_factuur_voor_week(INTEGER, TEXT) IS
  'Mig 232: genereer wekelijkse verzamelfactuur voor (debiteur, ISO-week). '
  'Aggregeert alle Verzonden orders van die week + voegt per bundel-zending '
  'een VERZEND-regel toe met drempel-toets (klant.verzend_drempel + '
  'gratis_verzending). Volgt mig 227 no-op-guard pattern. Aangeroepen door '
  'edge function factuur-verzenden bij queue-rij type=''wekelijks''.';

GRANT EXECUTE ON FUNCTION genereer_factuur_voor_week(INTEGER, TEXT)
