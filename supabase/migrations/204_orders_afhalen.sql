-- Migratie 204: orders.afhalen-vlag + RPC-uitbreiding
--
-- Twee kleine maar concrete uitbreidingen voor de order-module:
-- (1) Boolean op orders die markeert dat de klant zelf afhaalt — UI gebruikt
--     dit om de automatisch-toegevoegde verzendkosten-regel te onderdrukken
--     en magazijn/logistiek toont zo dat een order niet via vervoerder gaat.
-- (2) RPC's `create_order_with_lines` + `update_order_with_lines` lezen het
--     veld uit de payload en zetten het op de orders-rij. Default false zodat
--     bestaande callers ongewijzigd blijven werken.
--
...
------------------------------------------------------------------------
ALTER TABLE orders
  ADD COLUMN IF NOT EXISTS afhalen BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN orders.afhalen IS
  'TRUE = klant haalt order zelf op. UI onderdrukt dan automatische verzendkosten-regel; '
  'logistiek/zending overslaat vervoerder-stap. Mig 204.';

------------------------------------------------------------------------
-- 2. RPC create_order_with_lines — afhalen meenemen
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- 3. RPC update_order_with_lines — afhalen alleen aanpassen als key meegestuurd
------------------------------------------------------------------------
...
COMMENT ON FUNCTION create_order_with_lines(JSONB, JSONB) IS
  'Maakt order + regels atomair. Sinds mig 152: lever_modus. Sinds mig 194: maatwerk_band_kleur_id. Sinds mig 204: afhalen.';
COMMENT ON FUNCTION update_order_with_lines(BIGINT, JSONB, JSONB) IS
  'Update order header + replace regels atomair. Sinds mig 152: lever_modus. Sinds mig 194: maatwerk_band_kleur_id. Sinds mig 204: afhalen (alleen aanpassen als key in p_header staat).';
