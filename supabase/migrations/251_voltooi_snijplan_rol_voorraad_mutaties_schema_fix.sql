-- Migratie 251: voltooi_snijplan_rol — juiste voorraad_mutaties-kolommen bij aangebroken-rol
--
-- Symptoom (gerapporteerd 2026-05-11):
--   bij "Rol afsluiten" op een rol die als aangebroken wordt achtergehouden
--   (bijv. 400×700 cm volle breedte) gooit de RPC:
--     `column "lengte_voor_cm" of relation "voorraad_mutaties" does not exist (42703)`
--   Hele functie rolt terug → niets gebeurt: rol blijft op `snijden`, snijplannen
--   blijven op `Snijden`, reststuk wordt niet aangemaakt.
--
-- Root cause:
--   Mig 090 (en de re-definitie in mig 246, die alleen DELETE→TRUNCATE wijzigde)
--   schrijven:
--     INSERT INTO voorraad_mutaties (rol_id, type, lengte_voor_cm, lengte_na_cm,
--                                    reden, medewerker)
--     VALUES (..., 'aangebroken', ...);
--   Die kolommen bestaan niet en `'aangebroken'` zit niet in het type-CHECK.
--   De werkelijke tabel (mig 032, commit ece9ecd) heeft:
--     rol_id, type CHECK ('inkoop','snij','reststuk','correctie','afgekeurd'),
--     lengte_cm NOT NULL, breedte_cm, referentie_id, referentie_type,
--     notitie, aangemaakt_door.
--   Mig 136 fixte exact deze foute kolomnamen al voor `boek_ontvangst`; voor
--   `voltooi_snijplan_rol` was dat blijven liggen omdat de aangebroken-branch
--   pas geraakt wordt als de magazijnier "behoud rol (aangebroken)" kiest.
--
-- Fix:
--   * type='correctie' (bestaande toegestane waarde — een rol-lengte-correctie)
--   * lengte_cm = nieuwe (verkorte) rol-lengte
--   * breedte_cm = onveranderde rol-breedte (volle breedte blijft)
--   * notitie = beschrijft van/naar incl. originele lengte voor audit-trail
--   * aangemaakt_door = p_gesneden_door
--   * referentie_id = p_rol_id, referentie_type = 'rol_aangebroken'
--
--   Rest van de functie identiek aan migratie 246 (TRUNCATE-fix behouden).
...
BEGIN
  ...
  --    Mig 251: voorraad_mutaties INSERT met juiste kolommen.
  -- ---------------------------------------------------------------------
  IF v_aangebroken THEN
    ...
    VALUES (
      p_rol_id, 'correctie', p_aangebroken_lengte, v_rol.breedte_cm,
      p_rol_id, 'rol_aangebroken',
      'Rol aangebroken na snijden: van ' || v_rol.lengte_cm || ' cm naar '
        || p_aangebroken_lengte || ' cm (volle breedte overgebleven)',
      p_gesneden_door
    );
  ...
  -- ---------------------------------------------------------------------
  -- 3. Reststukken JSONB-flow (identiek aan 088/090/246 incl. waarde-toerekening)
  -- ---------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------
  -- 4. Kostentoerekening per zojuist afgevinkt snijplan (088/090/246).
  -- ---------------------------------------------------------------------
...
COMMENT ON FUNCTION voltooi_snijplan_rol(BIGINT, TEXT, INTEGER, JSONB, BIGINT[], INTEGER) IS
  'Voltooit een rol: snijplannen → Gesneden (of terug naar Wacht bij partial), maakt reststuk-rollen (p_reststukken), rekent grondstofkosten toe (088), en optioneel (p_aangebroken_lengte ≥100) verkort de originele rol naar een aangebroken rol i.p.v. status=gesneden. Mig 246: TRUNCATE i.p.v. DELETE voor _reststuk_out (pg_safeupdate). Mig 251: voorraad_mutaties INSERT gebruikt nu de echte tabel-kolommen (lengte_cm/breedte_cm/notitie/referentie_id/aangemaakt_door, type=correctie) — de eerdere lengte_voor_cm/lengte_na_cm/reden/medewerker waren verzonnen en gooiden 42703 zodra een rol aangebroken werd afgesloten.';
