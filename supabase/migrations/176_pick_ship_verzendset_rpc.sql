-- Migratie 176: Pick & Ship verzendset - create_zending_voor_order aanscherpen
--
-- Context:
-- Pick & Ship maakt nu direct een verzendset (zending + stickers + pakbon).
-- De oorspronkelijke RPC uit mig 172 gebruikte `order_regels.aantal`, maar de
-- orderregelkolom heet in RugFlow `orderaantal`. Deze versie vult bovendien
-- `aantal_colli` en `totaal_gewicht_kg` voor de printset.
--
-- Idempotent: vervangt dezelfde functie-signature en behoudt return type BIGINT.
...
COMMENT ON FUNCTION create_zending_voor_order IS
  'Maakt of hergebruikt één zending voor een order. Migratie 176 gebruikt '
  '`order_regels.orderaantal` voor zending_regels.aantal en vult aantal_colli + '
  'totaal_gewicht_kg voor Pick & Ship stickers/pakbon. Returnt zendingen.id.';
