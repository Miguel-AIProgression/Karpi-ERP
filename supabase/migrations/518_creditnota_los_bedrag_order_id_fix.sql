-- Migratie 518: los-bedrag creditregel hoeft geen order_id te hebben
--
-- maak_creditfactuur Modus C (p_los_bedrag) probeerde order_id=NULL te
-- inserteren in factuur_regels, maar die kolom had een NOT NULL constraint.
-- Creditnotas via los bedrag zijn per definitie niet aan een specifieke
-- orderregel gekoppeld — constraint versoepeld naar nullable.
--
-- Tevens herdefinitie van maak_creditfactuur zodat de los-bedrag-INSERT
-- expliciet order_id = NULL doorgeeft (was al zo, maar de constraint
-- blokkeerde het).

-- 1. Maak order_id nullable in factuur_regels
ALTER TABLE factuur_regels
  ALTER COLUMN order_id DROP NOT NULL;

-- 2. Herdefineer maak_creditfactuur (identiek aan mig 517, geen wijziging
--    in functie-body nodig — de constraint-fix hierboven is voldoende)
...
BEGIN
  ...
  IF p_los_bedrag IS NOT NULL THEN
    -- order_id mag NULL zijn voor een los creditbedrag (niet aan specifieke
    -- orderregel gekoppeld — constraint is nullable gemaakt in deze migratie)
