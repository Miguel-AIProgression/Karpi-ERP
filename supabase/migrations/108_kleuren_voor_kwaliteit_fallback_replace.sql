-- Migration 108: kleuren_voor_kwaliteit — fix fallback-selectie in
-- eigen_maatwerk_artikel CTE.
--
-- Bug in 107: `ORDER BY p.kleur_code, (CASE WHEN product_type='overig' ...),
-- p.artikelnr` koos bij meerdere 'overig'-rijen de laagste artikelnr — bij
-- VELV 16 was dat 771160017 (VELVET TOUCH Contour, verkoopprijs=NULL) i.p.v.
-- 771169998 (VELV16MAATWERK, €24,26). Gevolg: COALESCE pakte NULL, UI viel
-- terug op equiv_m2_prijs (€19,86 van CISC).
--
-- Fix: prioriteer producten met 'MAATWERK' in karpi_code/omschrijving én
-- met ingevulde verkoopprijs. CREATE OR REPLACE (geen DROP nodig, signatuur
-- ongewijzigd) zodat view-dependencies niet breken.
