-- Migratie 315: voorraadposities — expose gereserveerd_migratie_m2 als veld
--
-- Body identiek aan mig 314 (die de blokkering-m² aftrekt van eigen_totaal_m2).
-- Mig 315 voegt één extra return-veld toe: gereserveerd_migratie_m2 = het actieve
-- migratie_blokkering-m² per (kwaliteit, genormaliseerde kleur). Daarmee kan de
-- frontend de reservering als aparte chip tonen i.p.v. alleen impliciet via het
-- verlaagde totaal. De aftrek-logica zelf (GREATEST op eigen_totaal_m2) en de
-- geblokkeerd-CTE zijn ongewijzigd t.o.v. mig 314 (ADR-0028).
--
...
COMMENT ON FUNCTION voorraadposities(TEXT, TEXT, TEXT) IS
  ...
  'Mig 315 voegt gereserveerd_migratie_m2 toe als apart return-veld voor de frontend-chip.';
...
DO $$
BEGIN
  RAISE NOTICE 'Migratie 315 toegepast: voorraadposities geeft gereserveerd_migratie_m2 terug (ADR-0028).';
END $$;
