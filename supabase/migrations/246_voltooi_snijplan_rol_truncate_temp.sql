-- Migratie 246: voltooi_snijplan_rol — TRUNCATE i.p.v. DELETE zonder WHERE
--
-- Symptoom (gerapporteerd 2026-05-11):
--   bij klik op "Rol afsluiten" → "DELETE requires a WHERE clause (21000)"
--
-- Root cause:
--   De `pg_safeupdate` extension blokkeert DELETE/UPDATE statements zonder
--   WHERE-clause als safety net tegen data-loss. In de body van
--   `voltooi_snijplan_rol` (mig 090) staat:
--
--       DELETE FROM _reststuk_out;
--
--   Dat is een TEMP-table (ON COMMIT DROP) waar we de rijen leegmaken voor
--   het nieuwe RETURN QUERY-resultaat, maar pg_safeupdate maakt geen
--   uitzondering voor temp-tables en gooit error code 21000.
--
-- Fix:
--   Vervang DELETE met TRUNCATE. TRUNCATE is atomisch, sneller op een temp-
--   table en wordt niet door pg_safeupdate geblokkeerd. Functionele equivalent
--   in deze context.
--
-- Verder identiek aan migratie 090 (zelfde signature, zelfde body voor de
-- rest). We re-definiëren de hele functie via CREATE OR REPLACE; geen DROP
-- nodig omdat de signature (6 args) niet wijzigt.
...
BEGIN
  ...
  -- Mig 244: TRUNCATE i.p.v. DELETE zonder WHERE (pg_safeupdate blokkeert
  -- DELETE zonder WHERE met error 21000, ook op temp-tables).
  ...
  -- ---------------------------------------------------------------------
  -- 1. Snijplan-status updates (identiek aan 088/090)
  -- ---------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------
  -- 3. Reststukken JSONB-flow (identiek aan 088/090 incl. waarde-toerekening)
  -- ---------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------
  -- 4. Kostentoerekening per zojuist afgevinkt snijplan (088/090).
  -- ---------------------------------------------------------------------
...
COMMENT ON FUNCTION voltooi_snijplan_rol(BIGINT, TEXT, INTEGER, JSONB, BIGINT[], INTEGER) IS
  'Voltooit een rol: snijplannen → Gesneden (of terug naar Wacht bij partial), maakt reststuk-rollen (p_reststukken), rekent grondstofkosten toe (088), en optioneel (p_aangebroken_lengte ≥100) verkort de originele rol naar een aangebroken rol i.p.v. status=gesneden. Mig 244 wijzigt DELETE FROM _reststuk_out naar TRUNCATE om pg_safeupdate-error 21000 te vermijden.';
