-- Migration 109: kleuren_voor_kwaliteit — fix CTE-prioriteit in
-- eigen_maatwerk_artikel.
--
-- Bug in 107/108: bij VELV 16 vielen meerdere producten binnen de filter
-- `product_type='overig' OR omschrijving ILIKE '%maatwerk%'`:
--   * 771160017 (VELVET TOUCH 16 Contour, product_type='overig', verkoopprijs=NULL)
--   * 771169998 (VELV16MAATWERK,          product_type='overig', verkoopprijs=24.26)
-- `ORDER BY kleur_code, (product_type='overig'?0:1), artikelnr` pakte de
-- laagste artikelnr (771160017 → NULL). COALESCE gaf dus NULL terug en de UI
-- viel onterecht door op equiv_m2_prijs (CISC €19,86).
--
-- Fix: prioriteer producten met 'MAATWERK' in omschrijving/karpi_code én
-- met ingevulde verkoopprijs. CREATE OR REPLACE — signatuur ongewijzigd.
...
eigen_maatwerk_artikel AS (
  ...
  ORDER BY
    p.kleur_code,
    -- 1. product dat expliciet 'MAATWERK' heet (meest autoritatief)
    (CASE WHEN p.omschrijving ILIKE '%MAATWERK%' OR p.karpi_code ILIKE '%MAATWERK%' THEN 0 ELSE 1 END),
    -- 2. product met ingevulde verkoopprijs (geen NULL)
    (CASE WHEN p.verkoopprijs IS NOT NULL THEN 0 ELSE 1 END),
    -- 3. product_type='overig' vóór andere types
    (CASE WHEN p.product_type = 'overig' THEN 0 ELSE 1 END),
    p.artikelnr
)
SELECT
