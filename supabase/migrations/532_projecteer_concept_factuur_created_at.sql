-- Migratie 532: toeslag-activatie op orders.created_at + toeslag_procent snapshot
--
-- Superset van mig 529 (projecteer_concept_factuur_toeslag). Twee wijzigingen:
--
-- 1. ACTIVATIE-CHECK: was CURRENT_DATE BETWEEN begindatum AND einddatum (factuurdatum).
--    Nieuw: BOOL_AND(o.created_at::date >= begindatum AND o.created_at::date <= einddatum)
--    over alle orders in de factuur. Rationale: de toeslag geldt voor orders die
--    in de afgesproken periode zijn aangemaakt — niet afhankelijk van wanneer de
--    factuur toevallig wordt aangemaakt/herberekend.
--    Edge-case wekelijkse factuur met orders van vóór/ná de periode: bewust genegeerd
--    (geen actieve wekelijkse-factuur-klanten met toeslag op dit moment).
--    BOOL_AND over lege set = NULL → evalueert als FALSE → veilig.
--    toeslag_einddatum NULL: COALESCE(..., 'infinity') → geen einddatum.
--    toeslag_begindatum NULL: COALESCE(..., 'infinity') → nooit actief (veiligheidsnet).
--
-- 2. toeslag_procent SNAPSHOT: facturen.toeslag_procent (mig 531) wordt gevuld met
--    debiteuren.toeslag_procent zodat de PDF het percentage kan tonen ('Zuschlag 4,5%')
--    zonder een live join naar debiteuren.
...
BEGIN
  ...
  -- Toeslag-activatie (mig 532): geldig als toeslag_actief=TRUE EN ALLE orders in de
  -- zending zijn aangemaakt binnen de periode begindatum..einddatum.
...
COMMENT ON FUNCTION public.projecteer_concept_factuur(bigint, bigint) IS
  'Mig 428, gefixt mig 449, toeslag mig 529, created_at-check + procent-snapshot mig 532. '
  'Projecteert een concept-factuur voor een zending. Herhaalbaar, geen side-effects. '
  'Toeslag actief als BOOL_AND(orders.created_at::date binnen begindatum..einddatum). '
  'Snapshot: toeslag_procent uit debiteuren → facturen.toeslag_procent (voor PDF-label).';
