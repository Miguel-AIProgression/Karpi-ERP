-- Migratie 268: korting-factuur-regels gespreid per order
--
-- Probleem na mig 264:
--   · BUNDELKORTING/DREMPELKORTING factuur-regels missen `order_nr` +
--     `uw_referentie`. UI valt terug op `#<order_id>`; PDF-template
--     `factuur-pdf` groepeert op `order_nr` en plaatst de korting daardoor
--     onder een leeg "Ons Ordernummer :"-blok.
--   · BUNDELKORTING factuur-regel zit op `v_order_ids[1]`, terwijl de
--     orderregel-mirror (zelfde mig 264) BUNDELKORTING op `v_order_ids[2..]`
--     plaatst. Factuur en order spreken elkaar tegen voor 2+ orders.
--
-- Fix: spreid de korting-factuur-regels symmetrisch met de orderregel-mirror —
-- DREMPELKORTING op order[1] (drempel-cadeau zit op de "winnende" verzending),
-- BUNDELKORTING per order[2..N] (één regel per order, elk −verzendkosten).
-- Vul `order_nr` + `uw_referentie` via lookup naar `orders`. Voor 2 orders
-- blijft het bedrag identiek aan vóór mig 268: −(N−1) × verzendkosten verdeeld
-- over N−1 regels = −verzendkosten per regel. Voor 3+ orders óók consistent.
--
-- VOORWAARDE: mig 265 + mig 266 + mig 267 toegepast.
...
COMMENT ON FUNCTION genereer_factuur_voor_bundel(BIGINT) IS
  'Mig 268: V2-layout + korting-factuur-regels gespreid per order (DREMPEL op '
  'order[1] bij gratis_drempel, BUNDEL per order[2..N]). order_nr + '
