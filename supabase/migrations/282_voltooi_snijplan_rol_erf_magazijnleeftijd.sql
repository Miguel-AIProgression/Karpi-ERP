-- Migratie 282: voltooi_snijplan_rol — reststukken erven in_magazijn_sinds van de moederrol
--
-- Context (ADR-0021): het materiaal in een reststuk is even oud/verkleurd als
-- de moederrol. De FIFO-leeftijdsklok mag dus NIET resetten bij snijden.
-- Body identiek aan mig 251; enige wijziging: beide reststuk-INSERTs krijgen
-- in_magazijn_sinds = v_rol.in_magazijn_sinds (erven van moederrol).
--
-- BEWUST ONGEMOEID:
--   * reststuk_datum blijft CURRENT_DATE op nieuwe reststukken — regel ~242-246
--     (`WHERE oorsprong_rol_id=p_rol_id AND reststuk_datum=CURRENT_DATE`) hangt
--     hiervan af om net-gesneden reststukken te vinden voor kostentoerekening.
--   * aangebroken-branch: dezelfde fysieke rol wordt in place ge-UPDATE't en
--     behoudt zo automatisch zijn eigen in_magazijn_sinds (geen nieuwe rij).
...
BEGIN
  ...
  -- ---------------------------------------------------------------------
  -- 1. Snijplan-status updates (identiek aan 088/090/246/251)
  -- ---------------------------------------------------------------------
  ...
  --    Aangebroken = zelfde fysieke rij → in_magazijn_sinds blijft staan.
  -- ---------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------
  -- 3. Reststukken JSONB-flow (identiek aan 251 + in_magazijn_sinds-erfgang)
  -- ---------------------------------------------------------------------
  IF p_reststukken IS NOT NULL AND jsonb_array_length(p_reststukken) > 0 THEN
    ...
    LOOP
      ...
      INSERT INTO rollen (rolnummer, artikelnr, kwaliteit_code, kleur_code,
                          ...
                          oorsprong_rol_id, reststuk_datum, in_magazijn_sinds, waarde)
      VALUES (v_reststuk_nr, v_rol.artikelnr, v_rol.kwaliteit_code, v_rol.kleur_code,
              ...
              'beschikbaar', p_rol_id, CURRENT_DATE, v_rol.in_magazijn_sinds,
  ...
  ELSE
    ...
    IF v_rest_lengte >= 100 THEN
      ...
      INSERT INTO rollen (rolnummer, artikelnr, kwaliteit_code, kleur_code, lengte_cm, breedte_cm,
                          oppervlak_m2, status, oorsprong_rol_id, reststuk_datum, in_magazijn_sinds, waarde)
      VALUES (v_reststuk_nr, v_rol.artikelnr, v_rol.kwaliteit_code, v_rol.kleur_code,
  ...
  -- ---------------------------------------------------------------------
  -- 4. Kostentoerekening per zojuist afgevinkt snijplan (088/090/246/251).
  --    reststuk_datum=CURRENT_DATE-filter ONGEMOEID (vindt net-gesneden rest).
  -- ---------------------------------------------------------------------
...
COMMENT ON FUNCTION voltooi_snijplan_rol(BIGINT, TEXT, INTEGER, JSONB, BIGINT[], INTEGER) IS
  'Voltooit een rol: snijplannen → Gesneden (of terug naar Wacht bij partial), '
  'maakt reststuk-rollen (p_reststukken), rekent grondstofkosten toe (088), en '
  'optioneel (p_aangebroken_lengte ≥100) verkort de originele rol naar een '
  'aangebroken rol. Mig 251: voorraad_mutaties met echte kolommen. Mig 282: '
  'nieuwe reststukken erven in_magazijn_sinds van de moederrol (FIFO-leeftijd '
  'reset NIET bij snijden, ADR-0021); reststuk_datum blijft CURRENT_DATE voor '
  'de kostentoerekening-filter.';

NOTIFY pgrst, 'reload schema';

DO $$
BEGIN
  RAISE NOTICE 'Migratie 282 toegepast: voltooi_snijplan_rol — reststukken erven in_magazijn_sinds (ADR-0021).';
END $$;
