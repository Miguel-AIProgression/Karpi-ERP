-- Migratie 450: orders.express + snijplanning_overzicht toont 'm
--
-- Fase 2 van het maatwerk-snijplanning-vervolgtraject (na Fase 1, mig 447):
-- handmatige "express"-vlag die in de snijplanner-sortering (sortPieces,
-- _shared/ffdh-packing.ts) de hoogste prioriteit krijgt. Spiegelt het
-- bestaande boolean-vlag-patroon van `orders.debiteur_zeker` (mig 322).

ALTER TABLE orders
  ADD COLUMN IF NOT EXISTS express BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN orders.express IS
  'Mig 450: handmatige vlag (planner/verkoper) — krijgt hoogste prioriteit in '
  'sortPieces() (_shared/ffdh-packing.ts), vóór grootte/oppervlak/afleverdatum.';

-- snijplanning_overzicht uitbreiden (additief, non-breaking — zelfde patroon
-- als mig 447: volledige mig-447-body + 1 kolom aan het einde).
