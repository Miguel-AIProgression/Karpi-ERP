-- Migratie 348: match_edi_artikel — maat-/vorm-suffix-guard op de token-match
--
...
COMMENT ON FUNCTION match_edi_artikel IS
  ...
  'mig 348: stap 3 weigert als de rest-tekst een maat-patroon (NNNxNNN) of '
  ...
  'vereist). Migratie 159, defensief uitgebreid in 162, suffix-guard in 348.';
...
BEGIN
  ...
  IF NOT ('155x230'  ~* '\d+\s*[x×]\s*\d+') THEN RAISE EXCEPTION 'Mig 348: maat-patroon vangt 155x230 niet'; END IF;
  IF NOT ('155 x 230' ~* '\d+\s*[x×]\s*\d+') THEN RAISE EXCEPTION 'Mig 348: maat-patroon vangt "155 x 230" niet'; END IF;
  IF NOT ('rund 160' ~* '\y(rund|rond|ovaal|oval)\y') THEN RAISE EXCEPTION 'Mig 348: vorm-patroon vangt "rund 160" niet'; END IF;
  ...
  IF 'GRIJS' ~* '\d+\s*[x×]\s*\d+' OR 'GRIJS' ~* '\y(rund|rond|ovaal|oval)\y' THEN
    RAISE EXCEPTION 'Mig 348: guard vangt onterecht een kleur-suffix';
  END IF;
  -- Geen substring-false-positives op vorm-woorden (woordgrens \y werkt):
  IF 'rondje' ~* '\y(rund|rond|ovaal|oval)\y' THEN
    RAISE EXCEPTION 'Mig 348: vorm-patroon matcht binnen een woord (woordgrens kapot)';
  END IF;
  -- Definitie-asserties:
  IF v_def NOT LIKE '%suffix-guard%' THEN
    RAISE EXCEPTION 'Mig 348: definitie bevat de suffix-guard niet';
  END IF;
  RAISE NOTICE 'Mig 348: alle asserties geslaagd — maat-suffix-guard actief op stap 3';
END $$;
